...
main_screen=pygame.display.set_mode((Board_width,Board_height))
...
manual_img = pygame.Surface((200, 80))
manual_img.fill((0, 128, 0))  # Green for manual button
auto_img = pygame.Surface((200, 80))
auto_img.fill((128, 0, 0))
'''
run=True
while run:
    main_screen.fill((52, 78, 91))

    draw_text("welcome to the snake game",font,text_col,100,100)
...
class autogame:
    ...
    def __init__(self) :
        ...
        pygame.display.set_caption('snake game-automode')
    ...
    def charkh(self):
        ...
        directions = [
        self.snake.direction,  # Current direction
        pygame.Vector2(-self.snake.direction.y, self.snake.direction.x),  # Turn right
        pygame.Vector2(self.snake.direction.y, -self.snake.direction.x),  # Turn left
        -self.snake.direction  # Reverse
        ]

        for direction in directions:
            new_head = body_copy[0] + direction

        # Check if the new position is valid (within bounds and not colliding with the body)
            if (0 <= new_head.x < self.board.colno and
                0 <= new_head.y < self.board.rowno and
                new_head not in body_copy):
            # Found a valid move
                body_copy.insert(0, new_head)
                body_copy.pop()  # Maintain the same length
                self.snake.body = body_copy[:]
                return

    # If no valid move is found, the snake is truly stuck
        print("Snake is trapped and cannot escape!")
