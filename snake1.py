...
class game:
    ...
    def auto(self): 
        ...
        '''  
        while self.path:
            self.snake.direction=pygame.Vector2(int(self.path[0][0]) - int(self.snake.direction.x),
                                                int(self.path[0][1]) - int(self.snake.direction.y))
            self.path.pop(0)
        '''
    ...
    def eat(self):
        ...
        if  (0 <= int(body_copy[0].x+ self.snake.direction.x) < self.board.colno 
            ...
            #print(int(self.snake.direction.y),int(self.snake.direction.x))
            self.snake.direction= pygame.Vector2(int(self.snake.direction.y),int(self.snake.direction.x))
            new_head=body_copy[0]+ self.snake.direction
            body_copy.insert(0,new_head)
            self.snake.body=body_copy[:]
    ...
    def run(self):
        while True:
            ...
            if self.snake.body[0]==self.fruit.pos :
                ...
                if self.path:            #self.estimate_util(self.snake.body)
                    #print('tadaa')
                    self.auto()
                    self.snake.move()
